...
def run_detection(image: np.ndarray, threshold: float) -> dict[str, Any]:
    ...
    # Optimized for free tier: reduce imgsz and max_det for faster inference
    results = model.predict(
        source=image,
        conf=threshold,
        iou=0.45,
        imgsz=480,  # Reduced from 640 for faster inference
        max_det=8,  # Reduced from 12
        verbose=False,
...
def delete_all_reports() -> dict[str, Any]:
    """Delete all reports and associated images."""
    with closing(get_conn()) as conn:
        # Get all image paths first
        rows = conn.execute("SELECT image_path FROM reports").fetchall()
...
def delete_report(report_id: str) -> dict[str, Any]:
    ...
    with closing(get_conn()) as conn:
        # Get image path
        row = conn.execute("SELECT image_path FROM reports WHERE id = ?", (report_id,)).fetchone()
        if not row:
